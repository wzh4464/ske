...
newframe = frame(new_elli, new_points(:, 1), new_points(:, 2), cframe.sourceimg, handle_index({new_points{:,2}},cframe.index,cframe.ANum));
end
...
function new_index = handle_index(groups, index, num)

% currentView is index removed of 0
% oringinLabel is finding arc_name for each arc
currentView = zeros(num, 1);
tmp_j = 1;
...
new_index = zeros(length(index), 1);

for i = 1:length(groups)
    new_index(currentView(groups{i})) = i;
end
